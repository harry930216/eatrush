...
- 權限管理 API(role_permission 由 seed 建立;店長改權限的介面留 V2)
...
- **Flyway / Testcontainers**:「一次性設定型」工具,各約半天預習(攤在 Step 2/6 前夜),換得 schema 版控與真 PostgreSQL 測試 — 皆職缺常見詞
...
erDiagram
    ...
    PERMISSION ||--o{ ROLE_PERMISSION : "矩陣"
    ...
    ROLE_PERMISSION {
        varchar role PK
        bigint permission_id FK "複合主鍵之二"
    }
    MEAL_ORDER {
        bigint id PK
        bigint member_id FK
...
### 4.4 permission(權限定義)— 為何存在:後台功能的權限清單。為何用表不用 enum 寫死:V2 加新權限(如 REPORT_VIEW)只 INSERT 一列,不改 schema 不改舊 code

| 欄位 | 存在的價值 |
|---|---|
| code UK | 機器讀的(`MENU_MANAGE`/`STOCK_MANAGE`/`ORDER_STATUS_MANAGE`),進 JWT authorities、進 @PreAuthorize;唯一鍵防重複定義 |
| name | 人讀的(「菜單管理」),V2 權限管理畫面直接用 |

### 4.5 role_permission(角色權限矩陣)— 為何存在:**這就是你說的「後台才需要矩陣管理」** — 角色與權限的多對多。改權限=改這張表的列,不改任何 code
...
- 權限 3 筆 + 矩陣:OWNER 全有;STAFF 只有 ORDER_STATUS_MANAGE
...
| 2 功能授權(RBAC) | 你的角色能用哪些後台功能? | **method security** | 登入時把 role_permission 矩陣載入 JWT authorities;API 標 `@PreAuthorize("hasAuthority('STOCK_MANAGE')")` |
| 3 資源規則 | 你的方案能點這道菜嗎? | 業務層(Validator 鏈) | `plan.level >= required_level` 比大小 |

- **RBAC 動態性的已知取捨**:權限在登入時載入 token — 店長改了 role_permission 矩陣,已登入者要**重新登入才生效**(JWT 無狀態的代價;每請求查 DB 可即時生效但多一次查詢,V1 不採)。面試被問「改權限何時生效」,這就是答案與取捨
...
3. **`BaseEntity` @MappedSuperclass**:member / menu_item / meal_order 三個具 id+createdAt 的實體適用,配 JPA auditing — abstract 正統用法之二。plan / permission / order_item **刻意不套**(§4 沒給它們 created_at:沒有審計需求的表不硬繼承 — 繼承也要 YAGNI)
...
| role_permission 不映射為 JPA entity | V1 只在登入時讀一次矩陣,一句 JOIN 查詢比 @EmbeddedId 複合主鍵映射便宜 — 資料不是都得變物件(V2 做權限管理 API 時再面對) |
...
**Step 2|建表與 seed(1 天)**
交付:Flyway V1(七張表,§4)+ V2(seed:2 方案 5 菜 3 權限 + 矩陣 + 4 帳號)。
判準:重建資料庫再啟動,表就在、seed 看得到(Flyway 從零重建)。
思考題:§4 每張表「為何存在」你能不看文件講一遍嗎?rejected 用 JSON 而 accepted 用表的理由?
坑:PostgreSQL JSONB 欄位寫法;Flyway 檔名雙底線(V1__);role_permission 不映射為 entity(§10 刻意不用清單),登入時 JOIN 查詢載矩陣即可。
...
交付:註冊/登入/發 token(authorities 含 role_permission 矩陣)、@PreAuthorize 上鎖、假會員全面換成 SecurityContext 真會員(Validator 鏈 Step 4 已完整,這步只換「會員從哪來」)。
...
6. **權限管理 API 與畫面**:店長動態調 role_permission 矩陣(V1 只 seed)
...
13. **老闆儀表板(dashboard)**:今日銷量、熱門菜、食材消耗 — 唯一「聚合型」投影端(GROUP BY 讀模型),§4.4 埋的 REPORT_VIEW 權限在此兌現;帶出讀模型(read model)面試話題
...
Java 21+Maven / **PostgreSQL 16(與 UcMarket 同引擎)** / Flyway+validate / 依層分包(by-layer)套件 / BIGINT id / **RBAC 矩陣(permission+role_permission)+方案等級欄** / 三角色 CUSTOMER-STAFF-OWNER / 佔位式冪等 client UUID / 部分成立+rejected JSON 快照 / 鎖排序鐵律(扣與加回皆升冪)/ 增量進貨 / **菜品圖只存 URL** / Validator 鏈+StockDeductionStrategy 介面+刻意不用清單 / Testcontainers 真 PostgreSQL / JUnit 多執行緒(k6 留 M4)/ SecurityFilterChain 新式+@EnableMethodSecurity+jjwt+BCrypt / access token only(權限進 authorities,改矩陣重登生效)/ 手寫 cache-aside / **前端原生三家兩頁 demo 載具(Spring static 同源,Step 9,可砍)** / API 契約 §14 定死 / CI=build+test / README 中文主英文括號
