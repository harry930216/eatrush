...
「已套用不可變」保護的對象=**你控制之外、已套用過該檔的資料庫**。判準:數受害者 — ≥1 個(prod/staging/隊友本機)→ roll forward 加新檔;0 個 → 改原檔+DROP 重播,歷史可重寫。與 git 同構:已 push 不 rebase、未 push 隨便改(前夜預習 1 那句類比的完整版)。本次:prod 無、隊友無、CI 未建(且 CI 用 Testcontainers 每次起全新 DB,天生不怕改檔)、V1/V2 尚未 commit — **受害者 0,改原檔**。順帶:設計期反悔不值得進 migration 史書 — 開 V3 的話,V1 讀起來是七表、讀到 V3 才知道五表=史書變雜訊。**v1.0 上線那天起受害者永遠 ≥1,一切只能 roll forward — 現在是還能重寫歷史的最後窗口。**
