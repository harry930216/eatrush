...
- [ ] **⚠️ 欠債:V1/V2 內 permission / role_permission 的「註解死碼」尚未刪除**(2026-07-10 因進度壓力決定延後)。註解不執行,功能無影響;但**刪它會再改一次 checksum → 必須再 drop 重播一輪(× 兩台機器)**。判準:v1.0 上線後受害者永遠 ≥1、只能 roll forward,**這是還能無痛清除的最後窗口** — 務必在 v1.0 前清掉。
...
> **2026-07-10 註:動態 RBAC 主線(觀念 14/15/20/21 + 熱置換 + 參數化鎖 + 我的提問全軌跡)已抽出成獨立完整版 → `2026-07-09-dynamic-rbac.md`(第四戰);本處觀念條目保留作時序摘要。**
...
### 觀念 20. 老師第二招:配置外部化(externalized config)+ 熱重載 — 檔案與 DB 是同格兩座位
「在伺服器做 IO 把他換掉」=矩陣寫成**不進 jar 的外部檔案**(`/config/permissions.yml`),改權限=改檔案+程式重讀(觸發三選一:輪詢 polling / NIO WatchService 檔案監聽 / 手動 admin API;純 Spring Boot **沒有內建**熱重載,要自己寫或上 Spring Cloud @RefreshScope)。**能做到熱置換,但**:①token 層照樣冷(觀念 14 兩層不變,已登入要重登)②三筆工程費躲不掉 — 重讀觸發、讀入綁 enum fail-fast(否則 typo 靜默失權回歸)、**換版本的併發安全**(immutable 矩陣+volatile 引用一次性替換=safe publication,讀者永不見一半;檔案替換=寫暫存再 rename 原子換)。**判準定位**:這招等於承認矩陣要 runtime 可寫 — Q3 的判決本來就是「搬出 code」,檔案與 DB 是**同一格(code 之外的可變儲存)的兩個座位**:檔案=單機/低頻/工程師手改的輕量位;**多實例=殺手題**(每台一份會漂移、共享磁碟=自製窮人 DB)、前端改、要審計 → DB 位(交易/行鎖/一份真相免費送)。加角色照舊卡觀念 15 的雷(Role enum 綁不上就炸)— 瓶頸在 role 有無 enum 化,不在矩陣存哪。**EatRush 結論:V1 改動頻率 0=YAGNI 照舊 enum;真要 runtime 寫,手上有 PG 直接去 DB 位。這招進工具箱 — 真主場是 feature flag/限流閾值/營運參數**(產品化形態=Nacos/Apollo/配置中心)。
**追打「前端改組態檔=小事」(2026-07-09 續)**:happy path 確實幾行(API→驗證→寫檔→重讀),但兩個 admin 同時按儲存=讀整份 yaml→改一格→整份寫回=**read-modify-write lost update 原地復活**(UcMarket 進貨的老朋友)— 且 DB 有條件更新/行鎖可救,檔案什麼都沒有;加上原子替換/審計/寫入驗證/「改權限的權限」自指問題,每項 DB 免費送、檔案手刻。判準:**「小事」不能用 happy path 行數估,要用「誰會同時用它、壞了會怎樣」估**。前端改權限的真.小事版=前端+DB 矩陣(§15-6),不是前端+檔案。

### 觀念 21. 動態 RBAC 升級階梯 — 每階有自己的觸發,不一步跳頂(2026-07-09 拍板路線)
**V1 enum(0 表)→ ①DB 2 表:前端可「改分配」(觸發=前端出現儲存權限按鈕;搬家費≈0,§15-6)→ ②3 表+徹底去 enum 化:前端可「加角色」(觸發=真的要在前端生角色;價=頻率交換 — 全部角色判斷句退化 runtime、角色須純權限包、嚴守不建雙軌 enum)→ ③參數化鎖(通用攔截器+規則表):可熱改「既有門的齒形要求」(價=異形鎖出局如 cancel 的本人 OR 權限、規則表成單點、審計從讀 code 變查庫)→ 再上去(加權限+加門)=做不到,門是 code — 做到=在做 IAM 產品(Keycloak)。**
配套判準:①延遲每階決策到**最後責任時刻(last responsible moment)**— 成立前提=搬家費已知且低(付不起搬家費的才要預付,同 Q1 矩陣邏輯);②「之後再處理併發」的正解=**併發需求出現時換座位**(去天生有交易/行鎖的 DB),不是在檔案上手刻併發控制;③外置檔+熱重載這站對本案是多餘中繼(有 PG 直接跳 DB)— 想練熱重載=標記為練習圈支線,架構需要與學習想要分開記帳;④權限資料的熱度判準:**發 token 時讀一次(鑰匙串)=改了要重登;每次請求讀(規則表)=改了立即生效**。

### 五關分析作業(前端動態 RBAC)

**已移至 `2026-07-09-dynamic-rbac.md` §9**(主題歸位;該檔含五關題目、已教掉部分的標注、AI 補的「考慮不完備」清單 §8)。分析寫在那裡,寫完丟回來審。
